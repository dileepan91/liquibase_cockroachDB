...
public class Puzzle4 {
    public static void main(String[] args) {
        ...
        List<String> keys = Arrays.asList("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid");
        List<String> eclVal = Arrays.asList("amb", "blu", "brn", "gry", "grn", "hzl", "oth");
        ArrayList<String> v1 = new ArrayList<>();
        ArrayList<String> v2 = new ArrayList<>();
        int part1 = 0;
        int part2 = 0;
        for (String nl : newLineSplit) {
            if (nl.isEmpty()) {
                if (v1.containsAll(keys)) {
                    part1++;
                }
                if (v2.containsAll(keys)) {
                    part2++;
                }
                v1 = new ArrayList<>();
                v2 = new ArrayList<>();
                ...
                for (String s : sp) {
                    ...
                    switch (kv[0]) {
                        case "byr":
                            if (kv[1].matches("\\d+") && kv[1].length() <= 4) {
                                int y = Integer.valueOf(kv[1]);
                                if (y >= 1920 && y <= 2002) {
                                    v2.add(kv[0]);
                                }
                            }
                        ...
                        case "eyr":
                            ...
                            break;
                        case "hgt":
                            if (kv[1].matches("(1[5-8][0-9]|19[0-3])(cm)") || kv[1].matches("([5-6][0-9]|7[0-6])(in)")) {
                                v2.add(kv[0]);
                            }
                            break;
                        case "hcl":
                            if (kv[1].matches("^#([a-f0-9]{6})")) {
                                v2.add(kv[0]);
                            }
                            break;
                        case "ecl":
                            if (eclVal.contains(kv[1])) {
                                v2.add(kv[0]);
                            }
                            break;
                        case "pid":
                            if (kv[1].matches("\\d+") && kv[1].length() == 9) {
                                v2.add(kv[0]);
                            }
                            break;
                    }
